...
class ReportsAPIClient(BaseAPIClient):
        ...
        try:
            ...
            if report_type not in self.REPORT_TYPES.values():
                validation_errors.append(
                    f"Invalid report_type. Valid types: {', '.join(self.REPORT_TYPES.values())}"
                )
    ...
    def get_report_document(self, report_document_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Unexpected error in get_report_document: {e}")
            return self._format_error_response(
                "unexpected_error",
                f"An unexpected error occurred: {str(e)}",
            )
    ...
    def get_reports(
        ...
        try:
            ...
            transformed_reports = [
                self._transform_report_response(report) for report in reports
            ]
            ...
            logger.error(f"Unexpected error in get_reports: {e}")
            return self._format_error_response(
                "unexpected_error",
                f"An unexpected error occurred: {str(e)}",
            )
